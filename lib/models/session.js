  ...
  static async get(ctx) {
    ...
    if (!session) {
      if (cookieSessionId) {
        ...
        try {
          // interim solution to build URL with pre-defined protocol because ctx.host does not return protocol
          const DEBUG_PROTOCOL = 'http://';
          const url = new URL(`${DEBUG_PROTOCOL}${ctx.request.header.host}${ctx.request.url}`)
          ...
          console.info(DEBUG_PROTOCOL, err);
          session = this.instantiate();
        }
      }
    }
